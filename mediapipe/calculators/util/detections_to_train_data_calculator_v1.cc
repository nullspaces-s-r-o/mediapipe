  ...
  if (detection.location_data().format() == LocationData::BOUNDING_BOX) {
    ...
    if (detection.location_data().relative_keypoints_size()) {
      for (int i = 0; i < detection.location_data().relative_keypoints_size();
        ...
        auto x_ = detection.location_data().relative_keypoints(i).x();
        auto y_ = detection.location_data().relative_keypoints(i).y();
        std::cout << "keypoint " << i << " " << x_ << " " << y_ <<  std::endl;
        keypoint_data->set_x(x_);
        keypoint_data->set_y(y_);
      }
    }
  }
}
