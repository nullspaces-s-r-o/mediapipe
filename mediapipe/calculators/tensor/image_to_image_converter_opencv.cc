...
#include "mediapipe/calculators/tensor/image_to_image_converter_opencv.h"
...
#include "mediapipe/calculators/tensor/image_to_image_converter.h"
...
class ImageToImageOpenCvConverter : public ImageToImageConverter {
 public:
  ImageToImageOpenCvConverter(BorderMode border_mode,
  ...
  absl::Status Convert(const mediapipe::Image& input, const RotatedRect& roi,
                       ...
                       mediapipe::ImageFrame& output_image) override {
    ...
    // const auto& output_shape = output_image..shape();
    // MP_RETURN_IF_ERROR(ValidateTensorShape(output_shape));

    const int output_height = output_image.Height(); // output_shape.dims[1];
    const int output_width = output_image.Width(); // output_shape.dims[2];
    const int output_channels = output_image.NumberOfChannels(); // output_shape.dims[3];
    const int num_elements_per_img =
        output_height * output_width * output_channels;
    // auto dst = ::mediapipe::formats::MatView(output_image);
    cv::Mat dst(output_image.Height(), output_image.Width(), CV_8UC3, output_image.MutablePixelData());
    ...
    cv::Mat transformed;
    cv::warpPerspective(*src, dst, projection_matrix,
    ...
    //                                 range_min, range_max));
    // transformed.convertTo(dst, dst.type(), transform.scale,
    //                       transform.offset);
...
absl::StatusOr<std::unique_ptr<ImageToImageConverter>> CreateOpenCvConverterImage(
  ...
  return std::make_unique<ImageToImageOpenCvConverter>(border_mode,
                                                        tensor_type, flags);
}
