...
//   }
// }
class ImageToImageCalculator : public Node {
 public:
  ...
  static constexpr Input<mediapipe::NormalizedRect>::Optional kInNormRect{
      "NORM_RECT"};
  static constexpr Output<mediapipe::ImageFrame> kOutImage{"IMAGE"};
  ...
  MEDIAPIPE_NODE_CONTRACT(kIn, kInGpu, kInNormRect, kOutImage,
  ...
  absl::Status Process(CalculatorContext* cc) {
    ...
    Tensor tensor(
      ...
      // auto output_image = kOutImage(cc);
    // auto output_image = cc->Outputs().Tag(kOutImage);

    mediapipe::ImageFrame output_frame(image->image_format(), tensor_width, tensor_height);
    

    MP_RETURN_IF_ERROR((/* image->UsesGpu() ? gpu_converter_ :  */cpu_converter_)
                           ->Convert(*image, roi, params_.range_min,
                                     params_.range_max,
                                     /*tensor_buffer_offset=*/0, output_frame));
    ...
    // }
    kOutImage(cc).Send(std::move(output_frame));
    return absl::OkStatus();
  }
...
// made available even with MEDIAPIPE_DISABLE_GPU set.
  ...
  std::unique_ptr<ImageToImageConverter> cpu_converter_;
  mediapipe::ImageToTensorCalculatorOptions options_;
  OutputTensorParams params_;
  MemoryManager* memory_manager_ = nullptr;
};

MEDIAPIPE_REGISTER_NODE(ImageToImageCalculator);
