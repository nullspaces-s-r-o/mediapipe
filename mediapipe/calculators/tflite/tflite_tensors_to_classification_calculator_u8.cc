...
//   }
// }
class TfLiteTensorsToClassificationCalculatorU8 : public CalculatorBase {
 public:
...
REGISTER_CALCULATOR(TfLiteTensorsToClassificationCalculatorU8);

absl::Status TfLiteTensorsToClassificationCalculatorU8::GetContract(
...
absl::Status TfLiteTensorsToClassificationCalculatorU8::Open(
...
absl::Status TfLiteTensorsToClassificationCalculatorU8::Process(
    CalculatorContext* cc) {
  ...
  GetQuantizationParams(raw_score_tensor, scale, zero_point);

  const uint8_t* raw_scores = raw_score_tensor->data.uint8;
  ...
  if (options_.binary_classification()) {
    ...
    class_first->set_score(DQ(raw_scores[0]));
    class_second->set_score(1. - DQ(raw_scores[0]));
    ...
    for (int i = 0; i < num_classes; ++i) {
      if (options_.has_min_score_threshold() &&
          DQ(raw_scores[i]) < options_.min_score_threshold()) {
        continue;
      }
      Classification* classification =
          classification_list->add_classification();
      classification->set_index(i);
      classification->set_score(DQ(raw_scores[i]));
...
absl::Status TfLiteTensorsToClassificationCalculatorU8::Close(
    CalculatorContext* cc) {
  return absl::OkStatus();
}
